...
int BlockAccess::search(int relId, Attribute *record, char attrName[ATTR_SIZE], Attribute attrVal, int op) {
    ...
    using linearSearch() */
    BlockAccess::linearSearch(relId, attrName, attrVal, op);
...
// This method deletes the relation with the name specified as argument.
// This involves freeing the record blocks and index blocks allocated to this relation,
// as well as deleting the records corresponding to the relation in the relation catalog and attribute catalog.
...
int BlockAccess::deleteRelation(char relName[ATTR_SIZE]) {
    ...
    */
    ...
    while (currentblocknum != -1) {
            RecBuffer currentbuff (currentblocknum);

            //get block header using blockbuffer.getheader
            HeadInfo header;
            currentbuff.getHeader(&header);
            currentblocknum = header.rblock;
            currentbuff.releaseBlock();
    }
    ...
    ***/
    ...
    while(true) {
        RecId attrCatRecId = linearSearch(ATTRCAT_RELID, RELCAT_ATTR_RELNAME, relNameAttr, EQ);
        ...
        */
        if (head.numEntries==0/*   header.numEntries == 0  */) {
            ...
            */
            ...
            if (head.rblock!=INVALID_BLOCKNUM/* header.rblock != -1 */) {
